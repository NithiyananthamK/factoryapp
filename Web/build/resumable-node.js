var fs = require('fs'), path = require('path'), util = require('util'), Stream = require('stream').Stream,concatfiles = require('concat-files'),uuidv1 = require('uuid/v1');
//  axios = require('axios');
...
module.exports = resumable = function(temporaryFolder){
  ...
  $.post = function(req, callback){
    console.log('ENTERED RESUMABLE JS')
    ...
    if(validation=='valid') {
      var chunkFilename_extension = filename.substr(filename.lastIndexOf('.') + 1);
      var chunkFilename = getChunkFilename(chunkNumber, identifier);
	console.log("saving files from "+ files[$.fileParameterName].path + " to :"+ chunkFilename);
      ...
      fs.readFile(files[$.fileParameterName].path, function (err, data) {
        ...
        // Write the file
        var uuid = uuidv1()
        // +'-'+uuid+'.'+chunkFilename_extension
        fs.writeFile(chunkFilename, data, function (err) {
            if (err) throw err;
             // Do we have all the chunks?
              var currentTestChunk = 1;
              var numberOfChunks = Math.max(Math.floor(totalSize/(chunkSize*1.0)), 1);
              var testChunkExists = function(fields){
                    fs.exists(getChunkFilename(currentTestChunk, identifier), function(exists){
                      if(exists){
                        currentTestChunk++;
                        if(currentTestChunk>numberOfChunks) {
                          var destpath = path.join($.temporaryFolder,  "resumable-"+identifier+'-'+uuid+"."+chunkFilename_extension);
                          var destFile = "resumable-"+identifier+'-'+uuid+"."+chunkFilename_extension
                          mergeFiles(identifier,numberOfChunks, destpath)
                          callback('done', filename, original_filename, destFile);
                          // axios.get(fields.uploaddownloadservice+"/mergefiles?"+serialize(fields))

                        } else {
                          // Recursion
                          testChunkExists(fields);
                        }
                      } else {
                        callback('partly_done', filename, original_filename, "resumable-"+identifier+'-'+uuid+"."+chunkFilename_extension);
                      }
                    });
                  }
              testChunkExists(fields);
  ...
  $.mergeFiles = function(identifier,resumableTotalChunks, destPath){
    //destPath = "/home/dev/CRM/newCRM/curacrm/web/uploadFiles/test.png";
    //var fields = req.body;
    //var chunkNumber = fields['resumableChunkNumber'];
    //var identifier = cleanIdentifier(fields['resumableIdentifier']);
    var filelist = [];
    for(var i=1;i<=resumableTotalChunks;i++){
      filelist.push(getChunkFilename(String(i), identifier));

    }
    
    console.log('destPath :'+ destPath);
    concatfiles(filelist,destPath, function(err) {
      if (err) throw err
      console.log('done');
      filelist.forEach(function(filepath){
        fs.unlink(filepath, function(err) {
          if (err) {
            console.log('unable to delete tmp file : ' + filepath);
          } else if (i <= 0) {
            console.log('delted tmp file');
          }
        });
      });
